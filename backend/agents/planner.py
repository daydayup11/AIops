...
**第三步：规划可视化意图**
用自然语言描述希望生成的图表，**禁止**指定列名或图表类型，只描述"想看什么"。
...
- 最多澄清5轮，之后必须强制给出分析方案
- analysis_plan.viz_intent 只描述意图，**禁止**出现列名（如 src_ip、total_bytes）或图表类型关键词（bar、line、pie）；例如："展示各IP的外发流量排名，突出Top10异常值；同时展示时序趋势"
"""
